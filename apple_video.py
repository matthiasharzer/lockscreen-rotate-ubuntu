...
class VideoManager:
    ...
    def change_video(self):
        ...
        videos = [p for p in self.videos_dir.rglob("*") if p.suffix.lower() in VIDEO_EXTENSIONS]
    ...
    def listen_for_unlock(self):
        ...
        for line in process.stdout:
            # 'boolean false' means the screensaver/lockscreen just deactivated (unlocked)
            if "boolean false" in line:
                self.change_video()

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Apple Wallpaper Lockscreen Rotate Daemon")
    parser.add_argument('--symlink', required=True, help="Path where the symlink should be created")
    parser.add_argument('--directory', required=True, help="The base path to discover video files")
